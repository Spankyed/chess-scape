...
function VideoInput (props){
	let { invalidUrl, currVideoId, setValidity, 
		setVideoThumb, setVideoFound, submit, isLoading, videoFound, options } = props
...
function Embed({currVideoId, videoList, setVideoData, isPlaying, stopLoading, options, play}){
	const addDataListener = _=> { //  adds an iframe Api listener to retrieve YT video info
		window.onmessage = (e) => { // no cleanup needed, overrides existing message handler
			const { event, id, info } = typeof e?.data === 'string' ? JSON.parse(e.data) : {};
			// console.log(JSON.parse(e.data))
			if (id != 1) return
			if (event == 'initialDelivery' && info.videoData.title) setVideoData(info.videoData)
			if (options.autoPlay && event == 'infoDelivery' && info.playerState === 0) {
		...
		{ !thumbVideoId ?
			...
			{ isLoading ?
				<div class='middle-icon' style="top: calc(30%);">
					...
					<svg class="video-loader" version="1.1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="70px" height="70px" viewBox="0 0 50 50" style="enable-background:new 0 0 50 50;"><path fill="#000" d="M43.935,25.145c0-10.318-8.364-18.683-18.683-18.683c-10.318,0-18.683,8.365-18.683,18.683h4.068c0-8.071,6.543-14.615,14.615-14.615c8.072,0,14.615,6.543,14.615,14.615H43.935z"><animateTransform attributeType="xml" attributeName="transform" type="rotate" from="0 25 25" to="360 25 25" dur="0.6s" repeatCount="indefinite"/></path></svg>
				</div> :
