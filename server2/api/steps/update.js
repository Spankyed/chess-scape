...
const AccountApplications = require("./AccountApplications")(
...
const updateApplicationWithDecision = (id, decision) => {
	if (decision !== "APPROVE" && decision !== "REJECT") {
		throw new Error(
			"Required `decision` parameter must be 'APPROVE' or 'REJECT'"
		);
	}

	switch (decision) {
		case "APPROVE":
			return AccountApplications.update(id, { state: "REVIEW_APPROVED" });
		case "REJECT":
			return AccountApplications.update(id, { state: "REVIEW_REJECTED" });
	}
};
...
const updateWorkflowWithReviewDecision = async (data) => {
	...
	const updatedApplication = await updateApplicationWithDecision(
		id,
		decision
	);
